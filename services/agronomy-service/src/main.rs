...
async fn main() -> Result<()> {
    ...
    shared_observability::init(shared_observability::ObservabilityConfig {
        service_name: "agronomy-service".into(),
    ...
    info!(version = env!("CARGO_PKG_VERSION"), "Starting agronomy-service");
    ...
    let port = std::env::var("agronomy_service_PORT")
        .unwrap_or_else(|_| "3004".into());
    let addr = format!("0.0.0.0:{}", port);
    let listener = tokio::net::TcpListener::bind(&addr).await?;
    info!(addr = %addr, "agronomy-service listening");
